...
public class Ant {
	...
	private AntCell computeRandomMovement(List<AntCell> possibleDirections) {
		...
		if (bestPossibleDirections.isEmpty()) {
			...
			int randomNum = rand
					.nextInt((bestPossibleDirections.size() - 1) + 1);
	...
	private boolean isInOrientation(AntCell cell) {
		...
		int distanceFromOrientation = Math.abs(orientation[0]
				- newCellOrientation[0])
				+ Math.abs(orientation[1] - newCellOrientation[1]);
