...
public class SugarCell extends Cell {
	...
	public SugarCell(int xCoordinate, int yCoordinate, int startingState,
			Grid grid, SugarSimulation sim) {
	...
	public void preUpdateCell() {
		...
		if (myCurrentState == 5 && myDirty == false) {
			if (mySim.myAgentSugars[getX()][getY()] == 0) {
				...
				if (moveCell == null) {
					int ran = (int) Math.floor(Math.random()
							* myNeighbors.size());
					moveCell = myNeighbors.get(ran);
				}
				System.out.println("(" + moveCell.getX() + ","
						+ moveCell.getY() + ") "
						+ mySim.mySugars[moveCell.getX()][moveCell.getY()]);
				int x = moveCell.getX();
				int y = moveCell.getY();
				...
				mySim.myAgentSugars[moveCell.getX()][moveCell.getY()] = mySim.myAgentSugars[getX()][getY()]
						- mySim.myMetabolism;
				moveCell.setMyDirty(true);
			}
		}
		if (mySim.myTimes[getX()][getY()] % mySim.myInterval == 0
				&& mySim.mySugars[getX()][getY()] < mySugarLimit
				&& myNextState != 5) {
					...
					for (int i = 0; i < x.length; i++) {
						...
						if (xCoordinate >= 0
							...
							neighbors.add(myGrid.getCellMatrix()
									.get(xCoordinate).get(yCoordinate));
						}
					}
				}
